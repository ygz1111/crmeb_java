...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginResponse login(LoginRequest loginRequest) {
        ...
        if (ObjectUtil.isNull(user)) {
            // 此账号未注册，自动注册新用户
            user = userService.registerAccount(account, spreadPid);
            isNewUser = true;
        }
        if (ObjectUtil.isNull(user.getStatus()) || !user.getStatus()) {
            throw new CrmebException("此账号被禁用");
        }

        // 校验密码
        String password = CrmebUtil.encryptPassword(loginRequest.getPassword(), account);
        if (isNewUser) {
            // 新注册用户，设置用户输入的密码
