...
public class OnePassUtil {
    ...
    public JSONObject getFrom(String url, MultiValueMap<String, Object> param, Map<String, String> header) {
        Object result = restTemplateUtil.getDataForm(url, param, header);
        return checkResult(result);
    }
    ...
    private JSONObject checkResult(String result) {
        if (StringUtils.isBlank(result)) {
            logger.error("一号通平台接口异常，没任何数据返回！");
        ...
        try {
            ...
            logger.error("一号通平台接口异常！", e);
            return new JSONObject();
        }
        if (OnePassConstants.ONE_PASS_ERROR_CODE.equals(jsonObject.getInteger("status"))) {
            logger.error("一号通平台接口{}", jsonObject.getString("msg"));
            return jsonObject;
        }
        return jsonObject;
    }

    private JSONObject checkResult(Object result) {
        if (ObjectUtil.isNull(result)) {
            logger.error("一号通平台接口异常，没任何数据返回！");
            return new JSONObject();
        }
        if (result instanceof JSONArray) {
            logger.warn("一号通平台返回了数组数据，已忽略");
            return new JSONObject();
        }
