...
export default {
    ...
    onLoad() {
        ...
        if (!this.isLogin) {
            this.$util.Tips({ title: '请先登录' }, () => { uni.reLaunch({ url: '/pages/index/index' }); });
        }
    },
    methods: {
        uploadImage() {
            ...
            uni.chooseImage({
                ...
                success: function (res) {
                    ...
                    uni.uploadFile({
                        url: 'http://localhost:20510/api/front/user/upload/image',
                        filePath: res.tempFilePaths[0], name: 'multipartFile',
            ...
            publishSecondHand(that.formData).then(res => {
                ...
                if (res.data && res.data.success) {
                    ...
                    uni.showModal({
                        ...
                        showCancel: false,
                        success: function () { uni.switchTab({ url: '/pages/secondhand/mylist' }); }
                    });
                } else {
                    that.$util.Tips({ title: res.message || '发布失败' });
                }
            }).catch(err => { uni.hideLoading(); that.submitting = false; that.$util.Tips({ title: '网络错误，请重试' }); });
        },
    }
}
</script>
